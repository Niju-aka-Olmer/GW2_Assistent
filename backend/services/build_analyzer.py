...
def analyze_build_text(name: str, profession: str, specializations: list, equipment: list, combined_stats: dict = None, metabattle_content: str = "") -> str:
    prompt = f"[АНАЛИЗ БИЛДА ПЕРСОНАЖА]\n\nИмя персонажа: {name}\nПрофессия: {profession}\n\n"

    if metabattle_content:
        prompt += f"[ИНФОРМАЦИЯ С METABATTLE]\n{metabattle_content}\n\n---\n\n"

    prompt += "СУММАРНЫЕ ХАРАКТЕРИСТИКИ (со всей экипировки):\n"
    if combined_stats:
        prompt += _format_stats(combined_stats) + "\n"
    else:
        prompt += "Нет данных\n"
